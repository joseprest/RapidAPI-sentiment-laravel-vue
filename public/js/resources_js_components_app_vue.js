...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  data: function data() {
    return {
      message: "The ONE drawback and suggestion I have is a home button or gesture somewhere in the app! When I am deep into the app it gets really cumbersome to get back to the main screen. Also the battery starts to drain quite quick, but other than that I LOVE your app!"
    };
  },
  methods: {
    getAnalysis: function getAnalysis() {
      ...
      return _asyncToGenerator( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_0___default().mark(function _callee() {
        var options, _yield$axios$request, data, strArr, fullText, keywords;
              ...
              case 0:
                options = {
                  ...
                  headers: {
                    ...
                    "X-RapidAPI-Key": "92f0438fb6msh5e21532eb0681f9p16500fjsna198866cbc85"
                  }
                };
                _context.next = 3;
                return axios__WEBPACK_IMPORTED_MODULE_1___default().request(options);

              case 3:
                _yield$axios$request = _context.sent;
                data = _yield$axios$request.data;
                strArr = _this.message.split(/(\s+)/);
                console.log(data);
                fullText = _this.message;
                keywords = data.keywords;
                keywords.map(function (item) {
                  var index = strArr.findIndex(function (str) {
                    return str.toLowerCase() === item.word.toLowerCase();
                  });

                  if (index > 0) {
                    if (item.score > 0) {
                      fullText = fullText.replace(new RegExp("\\b" + item.word + "\\b", "gi"), "<span class='green-sen'>" + strArr[index] + "</span>");
                    } else {
                      fullText = fullText.replace(new RegExp("\\b" + item.word + "\\b", "gi"), "<span class='red-sen'>" + strArr[index] + "</span>");
                    }
                  }
                });
                _this.message = fullText;
